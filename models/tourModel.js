...
const tourSchema = new mongoose.Schema({
  name: {
    ...
    required: [true, 'A tour must nave a name'],
    unique: true,
    trim: true,
    ...
    // this validator was used to demo third pary validators
    // but its use was abandoned, since it does not allow spaces. 
  },
  slug: String,
  duration: {
    type: Number,
  ...
  ratingsAverage: {
    ...
    // 4.66666 (*10) = 46.666 (round) =  47 (/10) = 4.7
...
// but not for .insertMany(), not findOneUpdate(), , not InsertOne()
...
//     this.pipeline().unshift({ $match: { secretTout: {$ne: true} } });
